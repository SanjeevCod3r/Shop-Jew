import { create } from 'zustand';
import { persist, createJSONStorage } from 'zustand/middleware';

export const useAuthStore = create(
  persist(
    (set) => ({
      user: null,
      token: null,
      hydrated: false,
      setAuth: (user, token) => set({ user, token }),
      logout: () => set({ user: null, token: null }),
      setHydrated: (state) => set({ hydrated: state }),
    }),
    {
      name: 'auth-storage',
      storage: createJSONStorage(() => typeof window !== 'undefined' ? localStorage : {
        getItem: () => null,
        setItem: () => {},
        removeItem: () => {},
      }),
      onRehydrateStorage: (state) => (rehydratedState) => {
        if (rehydratedState) {
          rehydratedState.setHydrated(true);
        }
      },
    }
  )
);

export const useCartStore = create(
  persist(
    (set, get) => ({
      cartItems: [],
      cartCount: 0,
      hydrated: false,
      setCart: (items) => set({ cartItems: items, cartCount: items.length }),
      clearCart: () => set({ cartItems: [], cartCount: 0 }),
      setHydrated: (state) => set({ hydrated: state }),
      addItem: (product, quantity = 1, size = null) => {
        const currentItems = get().cartItems;
        const normSize = size == null || size === '' ? null : String(size);
        const existingItem = currentItems.find((item) => {
          const sameProduct =
            item.productId?._id === product._id || item.productId === product._id;
          const itemSize =
            item.size == null || item.size === '' ? null : String(item.size);
          return sameProduct && itemSize === normSize;
        });

        let newItems;
        if (existingItem) {
          newItems = currentItems.map((item) => {
            const sameProduct =
              item.productId?._id === product._id || item.productId === product._id;
            const itemSize =
              item.size == null || item.size === '' ? null : String(item.size);
            if (sameProduct && itemSize === normSize) {
              return { ...item, quantity: item.quantity + quantity };
            }
            return item;
          });
        } else {
          newItems = [
            ...currentItems,
            {
              productId: product,
              quantity,
              price: product.price,
              size: normSize,
            },
          ];
        }
        set({ cartItems: newItems, cartCount: newItems.length });
      },
      removeItem: (productId, size = undefined) => {
        const newItems = get().cartItems.filter((item) => {
          const pid = item.productId?._id || item.productId;
          if (pid !== productId) return true;
          if (size === undefined) return false;
          const itemSize =
            item.size == null || item.size === '' ? null : String(item.size);
          const target =
            size == null || size === '' ? null : String(size);
          return itemSize !== target;
        });
        set({ cartItems: newItems, cartCount: newItems.length });
      },
      updateQuantity: (productId, quantity, size = null) => {
        const target =
          size == null || size === '' ? null : String(size);
        const newItems = get()
          .cartItems.map((item) => {
            const sameProduct =
              item.productId?._id === productId || item.productId === productId;
            const itemSize =
              item.size == null || item.size === '' ? null : String(item.size);
            if (sameProduct && itemSize === target) {
              return { ...item, quantity: Math.max(0, quantity) };
            }
            return item;
          })
          .filter((item) => item.quantity > 0);
        set({ cartItems: newItems, cartCount: newItems.length });
      },
    }),
    {
      name: 'cart-storage',
      storage: createJSONStorage(() => typeof window !== 'undefined' ? localStorage : {
        getItem: () => null,
        setItem: () => {},
        removeItem: () => {},
      }),
      onRehydrateStorage: (state) => (rehydratedState) => {
        if (rehydratedState) {
          rehydratedState.setHydrated(true);
        }
      },
    }
  )
);
